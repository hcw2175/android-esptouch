...
import com.hcw2175.esptouch.util.CRC8;
import com.hcw2175.esptouch.util.EspNetUtil;

import java.net.InetAddress;

public class DatumCodeTransfer implements CodeTransfer {
	...
	private final DataCodeTransfer[] mDataCodes;
	 ...
	 */
	public DatumCodeTransfer(String apSsid, String apBssid, String apPassword,
		...
		// build data codes
		mDataCodes = new DataCodeTransfer[totalLen];
		mDataCodes[0] = new DataCodeTransfer(_totalLen, 0);
		totalXor ^= _totalLen;
		mDataCodes[1] = new DataCodeTransfer(apPwdLen, 1);
		totalXor ^= apPwdLen;
		mDataCodes[2] = new DataCodeTransfer(apSsidCrc, 2);
		totalXor ^= apSsidCrc;
		mDataCodes[3] = new DataCodeTransfer(apBssidCrc, 3);
		totalXor ^= apBssidCrc;
		mDataCodes[4] = null;
		for (int i = 0; i < ipLen; ++i) {
			mDataCodes[i + EXTRA_HEAD_LEN] = new DataCodeTransfer(ipAddrChars[i], i + EXTRA_HEAD_LEN);
		...
		for (int i = 0; i < apPwdChars.length; i++) {
			mDataCodes[i + EXTRA_HEAD_LEN + ipLen] = new DataCodeTransfer(
		...
		if (isSsidHiden) {
			for (int i = 0; i < apSsidChars.length; i++) {
				mDataCodes[i + EXTRA_HEAD_LEN + ipLen + apPwdLen] = new DataCodeTransfer(
						apSsidChars[i], i + EXTRA_HEAD_LEN + ipLen + apPwdLen);
			}
		}
		
		// set total xor last
		mDataCodes[4] = new DataCodeTransfer(totalXor, 4);
	}
	
	@Override
	public byte[] getBytes() {
		byte[] datumCode = new byte[mDataCodes.length * DataCodeTransfer.DATA_CODE_LEN];
		for (int i = 0; i < mDataCodes.length; i++) {
			System.arraycopy(mDataCodes[i].getBytes(), 0, datumCode, i
					* DataCodeTransfer.DATA_CODE_LEN, DataCodeTransfer.DATA_CODE_LEN);
		}
		return datumCode;
	}
